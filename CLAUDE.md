...
## Available Reference Codebases

When working on this project, you have access to several reference codebases for patterns and examples:

### 1. OpenAI Agents Python SDK (`openai-agents-python/`)
- **Location**: `openai-agents-python/` directory
- **Purpose**: Core OpenAI agents framework and examples
- **Key areas**: 
  - `src/agents/` - Core agent implementation
  - `examples/` - Various agent patterns and use cases
  - `examples/research_bot/` - Research agent examples
  - `examples/financial_research_agent/` - Multi-agent research patterns

### 2. Temporal Python SDK (`temporal-sdk-python/`)
- **Location**: `temporal-sdk-python/` directory  
- **Purpose**: Temporal workflow orchestration SDK
- **Key areas**:
  - `temporalio/` - Core Temporal SDK
  - `temporalio/contrib/openai_agents/` - OpenAI agents integration
  - `tests/` - Examples of workflow patterns, updates, queries

### 3. Wealth Management Example (`tmp_openai_agent_test/oai-temporal-wealth-management/`)
- **Location**: `tmp_openai_agent_test/oai-temporal-wealth-management/`
- **Purpose**: Complete example of Temporal workflows with OpenAI agents
- **Key areas**:
  - `src/temporal_supervisor/supervisor_workflow.py` - Workflow updates pattern
  - Shows how to use `@workflow.update` for interactive user input
  - Demonstrates proper Temporal workflow state management

### 4. Enhanced Research Workflow (Current Work)
- **Status**: Recently enhanced with clarifying questions functionality
- **Documentation**: See `openai_agents/workflows/research_agents/README.md` for details
- **Key innovation**: Multi-agent flow with interactive clarifying questions before research
- **Pattern**: Triage → Clarifying → Instruction → Research agents
...
- Run `pyright` on files you edited or created to catch type issues:
```bash
source .venv/bin/activate && pyright openai_agents/workflows/research_agents/clarifying_agent.py openai_agents/workflows/research_agents/instruction_agent.py openai_agents/workflows/research_agents/triage_agent.py openai_agents/workflows/research_agents/research_models.py openai_agents/workflows/research_agents/research_manager.py openai_agents/workflows/research_bot_workflow.py openai_agents/run_research_workflow.py openai_agents/test_enhanced_research.py
```
